...
static int invoke_procedure(struct jrpc_connection * conn, char *name, cJSON *params, cJSON *id) {
	cJSON *returned = NULL;
	...
	if (method != NULL && method->type == cJSON_String) {
		...
		|| params->type == cJSON_Object) {
			...
			//We have to copy ID because using it on the reply and deleting the response Object will also delete ID
				...
				if (conn->server->debug_level)
					printf("Method Invoked: %s\n", method->valuestring);
				return invoke_procedure(conn, method->valuestring,
						params, id_copy);
			}
		}
	}
...
static void close_connection(struct ev_loop *loop, struct jrpc_connection *conn) {
...
static void connection_cb(struct ev_loop *loop, ev_io *w, int revents) {
	...
	if (conn->pos == (conn->buffer_size - 1)) {
		...
		if (new_buffer == NULL) {
			perror("Memory error");
			return close_connection(loop, conn);
		}
...
static void accept_cb(struct ev_loop *loop, ev_io *w, int revents) {
	...
	struct jrpc_server *server = container_of(w, struct jrpc_server, listen_watcher);
	struct jrpc_connection *new_connection;
	new_connection = malloc(sizeof(struct jrpc_connection));
	
	struct sockaddr_storage their_addr; // connector's address information
	socklen_t sin_size;
	sin_size = sizeof their_addr;
	new_connection->io.fd = accept(w->fd, (struct sockaddr *) &their_addr,
			&sin_size);
	if (new_connection->io.fd == -1) {
		perror("accept");
		free(new_connection);
	} else {
		if (server->debug_level) {
		...
		ev_io_init(&new_connection->io, connection_cb,
				new_connection->io.fd, EV_READ);
		new_connection->buffer_size = 1500;
		new_connection->buffer = malloc(1500);
		memset(new_connection->buffer, 0, 1500);
		new_connection->pos = 0;
		new_connection->server = server;
		ev_io_start(loop, &new_connection->io);
	}
}
